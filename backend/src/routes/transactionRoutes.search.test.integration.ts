...
describe('POST /api/transactions (Integration)', () => {
    ...
    describe('filter date format support', () => {
        // All formats should treat the boundary date as 2025-03-01,
        // so the transaction on 2025-01-01 should be excluded in every case.
        beforeEach(async () => {
            await seedTransaction({ account: account1, upload: uploadRecord, amount: 10, date: '2025-01-01' });
            await seedTransaction({ account: account1, upload: uploadRecord, amount: 20, date: '2025-03-15' });
        ...
        it('accepts the typical client date format (YYYY-MM-DD)', async () => {
            const res = await request(app)
                .post('/api/transactions/')
                .set('Cookie', `session=${sessionToken}`)
        ...
        const supplementaryCases = [
            { label: 'ISO datetime without timezone (no offset)', dateFrom: '2025-03-01T00:00:00' },
            { label: 'ISO datetime with milliseconds, no offset', dateFrom: '2025-03-01T00:00:00.000' },
            { label: 'ISO datetime with Z offset', dateFrom: '2025-03-01T00:00:00Z' },
            { label: 'ISO datetime with explicit +00:00 offset', dateFrom: '2025-03-01T00:00:00+00:00' },
            { label: 'ISO datetime with ms and Z offset', dateFrom: '2025-03-01T00:00:00.000Z' },
        ];

        for (const { label, dateFrom } of supplementaryCases) {
            it(`also accepts ${label}`, async () => {
                const res = await request(app)
                    .post('/api/transactions/')
                    .set('Cookie', `session=${sessionToken}`)
                    .send({ filter: { dateFrom } })
                    .expect(200);
